...
namespace DevBurguer.Data
{
    public class PedidoRepository
    {
        ...
        {
            const string sql = "SELECT Id, Nome, Preco FROM Produtos";
            try
            {
            ...
            try
            {
                ...
                using (SqlConnection conn = DevBurguer.Banco.Conexao.GetConnection())
                {
                    ...
                    using (var tran = conn.BeginTransaction())
                    {
                        try
                        {
                            using (SqlCommand cmdPedido = new SqlCommand("INSERT INTO Pedidos (IdCliente, Total) OUTPUT INSERTED.Id VALUES (@c,@t)", conn, tran))
                            {
                                cmdPedido.Parameters.Add(new SqlParameter("@c", SqlDbType.Int) { Value = idCliente });
                                cmdPedido.Parameters.Add(new SqlParameter("@t", SqlDbType.Decimal) { Precision = 18, Scale = 2, Value = total });

                                idPedido = (int)await cmdPedido.ExecuteScalarAsync().ConfigureAwait(false);
                            }

                            foreach (var item in itens)
                            {
                                using (SqlCommand cmdItem = new SqlCommand("INSERT INTO ItensPedido (IdPedido, IdProduto, Quantidade, Observacao, Preco) VALUES (@p,@prod,@q,@obs,@preco)", conn, tran))
                                {
                                    ...
                                    cmdItem.Parameters.Add(new SqlParameter("@obs", SqlDbType.NVarChar, -1) { Value = (object)item.Observacao ?? string.Empty });
                                    cmdItem.Parameters.Add(new SqlParameter("@preco", SqlDbType.Decimal) { Precision = 18, Scale = 2, Value = item.Preco });

                                    await cmdItem.ExecuteNonQueryAsync().ConfigureAwait(false);
                                }
                            }
